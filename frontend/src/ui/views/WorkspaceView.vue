...
watch(canvasFullscreen, (on) => {
  if (on) {
    ...
    debugSidebarOpen.value = false
    return
  }
  // Herstel sidebar op alle canvas-stappen (niet project).
  if (ws.flowStep !== 'project') sidebarOpen.value = sidebarOpenBeforeFullscreen.value
})
...
const rasterCanUndo = computed(() => {
  if (ws.flowStep === 'input') return ws.canUndoMask
  if (ws.flowStep === 'preprocess' || ws.flowStep === 'templates') return ws.canUndoInkEdit
  return false
})

const rasterCanRedo = computed(() => {
  if (ws.flowStep === 'input') return ws.canRedoMask
  if (ws.flowStep === 'preprocess' || ws.flowStep === 'templates') return ws.canRedoInkEdit
  return false
})

const rasterHelpKeys = computed(() => workspaceCanvasHelpKeys(ws.flowStep))

function onRasterUndo() {
  if (ws.flowStep === 'input') {
    ws.undoMaskEdit()
    return
  }
...
<template>
  <div class="layout" :class="{ 'layout--canvas-fs': canvasFullscreen }">
    <div v-show="sidebarOpen" class="sidebar-wrap">
      <aside class="sidebar">
        <div class="sidebar-scroll sidebar-compact">
          ...
          <WorkspaceSidebarInputStep
            ...
            :baked-rotation-deg="ws.bakedInputRotationDeg"
          ...
          <WorkspaceSidebarPreprocessStep
            ...
            :wall-stamp-baked-inject-count="ws.wallStampBakedInjectCount"
            ...
            :wall-stamp-use-stamp-set="ws.wallStampUseStampSet"
          ...
          <WorkspacePlanResultPanel
            ...
            :underlay-available="!!ws.underlaySrc && !!(ws.planPlateLayout ?? ws.previewUnderlayLayout)"
    ...
    <div class="canvas-area">
      <button
        v-if="!sidebarOpen && !(ws.imageSrc && ws.flowStep !== 'project')"
        type="button"
        class="sidebar-fab"
      ...
      <WorkspaceFloorRail
        v-if="ws.flowStep !== 'project' && !canvasFullscreen"
        :floors="ws.projectFloors"
        :active-floor-id="ws.activeFloorId"
      ...
      <template v-else-if="ws.imageSrc && ws.flowStep !== 'project'">
        <div class="canvas-main">
          ...
          <WorkspacePlanCanvasHost
            ...
            :cm-origin="ws.planPlateLayout?.origin ?? null"
            :px-per-mm-x="ws.planPlateLayout?.pxPerMmX ?? 1"
            :px-per-mm-y="ws.planPlateLayout?.pxPerMmY ?? 1"
            :rotation-deg="ws.planPlateLayout?.rotationDeg ?? 0"
            :flip-x="ws.planPlateLayout?.flipX === true"
            :underlay-move-mode="ws.underlayMoveMode"
            :thickness-pick-tier="ws.thicknessPickTier"
          ...
          >
            <FloorplanCanvas
              ...
              :can-undo="rasterCanUndo"
              :can-redo="rasterCanRedo"
              :help-keys="rasterHelpKeys"
              ...
              :wall-stamp-allow-resize="ws.wallStampAllowResize"
              :show-guide-grid="ws.showCanvasGrid"
              :guide-grid-surface="ws.flowStep === 'input' ? 'input' : 'workspaceScan'"
              @update:show-guide-grid="ws.showCanvasGrid = $event"
              @undo="onRasterUndo"
              @redo="onRasterRedo"
    ...
    <WorkspaceDiagnosisFab
      :visible="!!ws.imageSrc && ws.flowStep !== 'project' && !canvasFullscreen"
