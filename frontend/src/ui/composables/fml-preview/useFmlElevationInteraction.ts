...
import { FML_PLAN_HANDLE_HIT_PX } from './fml-preview-vertex-hit'
import type { ElevTool } from './fml-elevation-tool'
import type { ElevationInteractionDeps } from './fml-elevation-interaction-types'
import { useFmlElevationDrawTools } from './useFmlElevationDrawTools'
import { useFmlElevationSelectEdit } from './useFmlElevationSelectEdit'
import { useFmlElevationPrecise } from './useFmlElevationPrecise'
...
} from './fml-elevation-interaction-types'

export function useFmlElevationInteraction(deps: ElevationInteractionDeps) {
  const {
    props,
    emit,
    elevation,
  ...
  const preciseBox: {
    current: ReturnType<typeof useFmlElevationPrecise> | null
  } = { current: null }

  const select = useFmlElevationSelectEdit({
    props,
    elevation,
    clientToCm,
    pointerCm,
  ...
  const precise = useFmlElevationPrecise({
    props,
    elevation,
    clientToCm,
    useTouchNav,
  ...
  const draw = useFmlElevationDrawTools({
    props,
    elevation,
    clientToCm,
    activeTool,
...
export type ElevationInteraction = ReturnType<typeof useFmlElevationInteraction>
