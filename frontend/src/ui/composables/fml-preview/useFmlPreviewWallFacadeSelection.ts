...
export type WallFacadeSelectionDeps = {
  editor: EditorApi
  selection: FmlPreviewSelectionRefs
  settingsWallSplitClickCm: Ref<Point2D | null>
  syncPlanToParent: () => void
  flushPendingFieldCommits: () => void
  syncFacadeThicknessDraftFromGroup: () => void
  syncWallThicknessDraftFromSelection: () => void
  /** Accepted from WallSelection; thickness sync stays there. */
  wallThicknessDraft?: Ref<number>
  wallThicknessMixed?: Ref<boolean>
}

/** Factory alias for `useFmlPreviewWallFacadeSelection` (WallSelection call site). */
export function createWallFacadeSelection(deps: WallFacadeSelectionDeps) {
  const {
    wallThicknessDraft: _wallThicknessDraft,
    wallThicknessMixed: _wallThicknessMixed,
    ...facadeDeps
  } = deps
  return useFmlPreviewWallFacadeSelection(facadeDeps)
}

/**
 * Gevelgroep- + stempel-selectie UI (checks, create/rename, member-pick).
 * Thickness-draft sync blijft in WallSelection (`syncFacadeThicknessDraftFromGroup`).
 */
export function useFmlPreviewWallFacadeSelection(options: {
  editor: EditorApi
  selection: FmlPreviewSelectionRefs
  settingsWallSplitClickCm: Ref<Point2D | null>
  syncPlanToParent: () => void
  ...
  facadeGroupOptions: ComputedRef<FacadeGroup[]>
  facadeGroupChecks: ComputedRef<Record<string, boolean | null>>
  stampGroupDraft: ComputedRef<boolean | null>
  stampGroupMixed: ComputedRef<boolean>
  facadeMemberIdsOnActiveFloor: ComputedRef<string[]>
  stampMemberIdsOnActiveFloor: ComputedRef<string[]>
  selectedFacadeGroupPanel: ComputedRef<{
    groupId: string
    name: string
    wallCount: number
    floorCount: number
  } | null>
  createFacadeGroupFromSelection: () => Promise<void>
  toggleFacadeGroup: (groupId: string, enabled: boolean) => Promise<void>
  removeFacadeGroupFromSelection: (groupId: string) => Promise<void>
  editAllFacadeGroups: () => Promise<void>
  applyFacadeGroupSelection: (value: string) => Promise<void>
  applyStampGroupSelection: (enabled: boolean) => void
  renameFacadeGroupById: (groupId: string) => Promise<void>
  renameSelectedFacadeGroup: () => Promise<void>
  selectFacadeGroupMembersById: (groupId: string) => void
  selectFacadeGroupMembers: () => void
  canSelectMembersOfGroup: (groupId: string) => boolean
  selectStampGroupMembers: () => void
  canSelectFacadeMembers: ComputedRef<boolean>
  canSelectStampMembers: ComputedRef<boolean>
} {
  const {
    editor,
    selection,
