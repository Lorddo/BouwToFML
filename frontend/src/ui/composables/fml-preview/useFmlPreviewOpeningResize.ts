...
} from './fml-preview-opening-handles'
...
export function useFmlPreviewOpeningResize(options: {
  editor: EditorApi
  selection: Pick<FmlPreviewSelectionRefs, 'moveOpeningId' | 'settingsOpeningIds'>
  ...
  function handleHitTolCm(): number {
    const fine = FML_PLAN_HANDLE_HIT_PX
    const coarse = FML_PLAN_HANDLE_HIT_COARSE_PX
