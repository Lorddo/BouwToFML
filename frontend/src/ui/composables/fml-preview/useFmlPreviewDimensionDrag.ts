...
import { offsetDimensionForSlide } from '@/core/fml/offset-dimension-line'
...
export function useFmlPreviewDimensionDrag(options: {
  clientToCm: (clientX: number, clientY: number) => Point2D | null
  editor: EditorApi
  selection: Pick<FmlPreviewSelectionRefs, 'moveDimensionId'>
  ...
  function beginDrag(id: string, event: MouseEvent): void {
    ...
    dimDrag = { id, base: { ...dim, a: { ...dim.a }, b: { ...dim.b } }, startCm: cm }
  ...
  function onDragMove(event: MouseEvent): void {
    ...
    const cm = clientToCm(event.clientX, event.clientY)
    if (!cm) return
    const next = offsetDimensionForSlide(dimDrag.base, {
