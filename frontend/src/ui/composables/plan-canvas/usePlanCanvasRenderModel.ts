...
import { listRidgeSurfacesOnFloor, isDormerRoof, resolveRoofSurfaceColor } from '@/core/plan/roof-planes'
import type { Floor, FloorPlan, FloorSurface, Wall } from '@/core/plan/types'
import { buildAutoDimensionLines } from '@/core/plan/auto-dimension-lines'
...
export type { RenderCornerMarker } from './plan-canvas-corner-markers'
...
export function usePlanCanvasRenderModel(
  ...
  const visibleJunctionIds = computed(() => {
    ...
    const showAllDrawJunctions =
      ...
      (view?.mode === 'dak' && selection.activePlanTool.value === 'draw_surface') ||
      selection.activePlanTool.value === 'draw_roof'
  ...
  const junctionMarkerRadius = computed(() => PLAN_HANDLE_RADIUS_PX)
  ...
  const selectedOpeningPanel = computed(() => {
    const model = renderModel.value
    if (!model) return null
    // Left-klik én Ctrl-klik: volle strook (type, scharnier, …). Kozijnrij alleen
    // bij Ctrl (`settingsOpeningIds`) — PlanCanvas zet showOpeningFrameEdit daarop.
