...
import {
  applyTwoFingerNav,
  PLAN_CANVAS_CHROME_SELECTOR,
  isTapMove,
  shouldUseTouchNav,
  type GesturePoint,
} from './plan-canvas-gestures'
import {
  PLAN_LIVE_POINTER,
...
} from './plan-canvas-touch-tap'
import { clampViewScale } from './usePlanCanvasPanZoom'
...
export function usePlanCanvasTouch(options: {
  ...
  function touchIgnoreTarget(event: PointerEvent): boolean {
    ...
    return Boolean(target?.closest(PLAN_CANVAS_CHROME_SELECTOR))
  }
...
export function usePlanTouchNav(touchEditor: ComputedRef<boolean> | Ref<boolean>): {
