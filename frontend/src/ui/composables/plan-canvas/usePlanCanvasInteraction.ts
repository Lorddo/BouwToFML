...
import { usePlanCanvasJunctionMove } from './usePlanCanvasJunctionMove'
import { usePlanCanvasOpeningMove } from './usePlanCanvasOpeningMove'
...
export function usePlanCanvasInteraction(options: {
  ...
  /** View-as: welke tab open staat. Vervangt de gedrilde `dakMode`-ref. */
  ...
  const wallDrag = usePlanCanvasWallDrag({
    hitTest,
    editor,
    selection,
    spacePressed,
  ...
  const wallMove = usePlanCanvasWallMove({
    hitTest,
    editor,
  ...
  const junctionMove = usePlanCanvasJunctionMove({
    hitTest,
    editor,
  ...
  const openingMove = usePlanCanvasOpeningMove({
    hitTest,
    editor,
  ...
  const itemDrag = usePlanCanvasItemDrag({
    hitTest,
    editor,
    selection,
    spacePressed,
  ...
  const areaLabelDrag = usePlanCanvasAreaLabelDrag({
    hitTest,
    editor,
    selection,
    spacePressed,
  ...
  const dimensionDrag = usePlanCanvasDimensionDrag({
  ...
  const selCoord = usePlanCanvasSelectionCoordinator({
    hitTest,
    selection,
    editor,
    snap,
  ...
  const toolCoord = usePlanCanvasToolCoordinator({
    hitTest,
    selection,
    editor,
    snap,
  ...
  watch(selection.activePlanTool, (tool) => {
    if (tool != null && isPreciseMoveDrafting()) cancelPreciseMoves()
  })

  // --- Inspect ---

  const inspect = usePlanCanvasInspect({
    hitTest,
    selection,
    walls: editor.walls,
  ...
  const itemResize = usePlanCanvasItemResize({
    editor,
  ...
  const itemRotate = usePlanCanvasItemRotate({
    editor,
    selectedItemId: selectedHandleItemId,
    settingsItemId: selection.settingsItemId,
    clientToCm: (x, y) => hitTest.clientToCm(x, y),
  ...
  const openingResize = usePlanCanvasOpeningResize({
    editor,
    selection,
    clientToCm: (x, y) => hitTest.clientToCm(x, y),
    screenPxToCm,
  ...
  const { onEditorKeyDown, onEditorKeyUp } = createPlanCanvasEditorKeyHandlers({
    selection,
    inspectMode,
    drawSurfaceMode,
    measureMode,
