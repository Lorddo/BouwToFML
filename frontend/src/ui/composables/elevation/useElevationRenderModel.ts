...
export function useElevationRenderModel(options: ElevationRenderModelOptions) {
  const {
    props,
    containerRef,
    settingsTarget,
  ...
  const viewport = usePlanCanvasViewport(containerRef, emptyWalls, undefined, extraBounds)
  const {
    stageSize,
    viewScale,
    viewPosition,
  ...
  const roofVertexHitTolCm = computed(() => {
    ...
    return PLAN_HANDLE_HIT_PX / scale
  })
  ...
  const elevationPaintStack = computed(() => {
    const planes = elevationPlanes.value
    const roofs = elevation.value?.roofPlanes ?? []
    const items = [
      ...planes.map((plane) => ({
        kind: 'plane' as const,
        key: plane.key,
      ...
      ...roofs.map((roof) => ({
        kind: 'roof' as const,
        key: `roof-${roof.floorIndex}-${roof.id}`,
        depthCm: roof.depthCm,
        dormer: roof.dormer,
        roof,
      })),
    ]
    items.sort(compareElevationPaintStackItems)
    return items
  })
...
export type ElevationRenderModel = ReturnType<typeof useElevationRenderModel>
