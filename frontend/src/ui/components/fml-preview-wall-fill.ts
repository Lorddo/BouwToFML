...
import type {
  ...
  WallPolygonInput,
  WallRenderGeometry,
} from '@/ui/components/fml-preview-wall-polygons-types'
 ...
 */
export const QUANTIZE_CM = 0.01
export const MIN_WALL_LENGTH_CM = 0.5

export function quantize(value: number): number {
  return Math.round(value / QUANTIZE_CM) * QUANTIZE_CM
}

export function quantizePoint(point: Point2D): Point2D {
...
export function leftNormal(dir: Point2D): Point2D {
...
export function ensureClosedRing(points: Point2D[]): Point2D[] {
...
export function toClippingRing(points: Point2D[]): [number, number][] {
...
export function fromClippingRing(ring: [number, number][]): Point2D[] {
...
export function toUnionGeom(points: Point2D[]): [[number, number][]] | null {
