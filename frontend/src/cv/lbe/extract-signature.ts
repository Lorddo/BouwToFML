...
import { inferWallRenderStyle, type InferWallRenderStyleOptions } from './infer-wall-render-style'
...
function wallStyleFields(
  ...
  return {
    parallelLineCount:
      renderStyle === 'parallel_lines' ? Math.max(2, parallelLineCount ?? 2) : undefined,
    parallelSpacingPx:
...
function makeWallSignature(
  ...
  return finalizeWallSignature({
    ...
    ...wallStyleFields(
      inferred.renderStyle,
      thicknessPx,
      measuredSpacing,
      inferred.parallelLineCount,
    ),
...
function makeDoorSignature(mat: CvMat, sample: ExampleSample): DoorSignature {
  ...
  const openingWidth = Math.max(
    ...
    Math.round(
      Math.max(measureLongestLightGapPx(mat, sample), Math.max(ink.width, ink.height) * 0.7),
    ),
  )
