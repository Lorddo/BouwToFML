...
function normalizeLimits(raw: Partial<FmlWallThicknessLimits> | null | undefined): FmlWallThicknessLimits {
  const minRaw = Number(raw?.minCm)
  const midRaw = Number(raw?.midCm)
  const maxRaw = Number(raw?.maxCm)
  const minCm = Number.isFinite(minRaw) && minRaw > 0 ? minRaw : DEFAULT_FML_WALL_THICKNESS_LIMITS.minCm
  const midCm = Number.isFinite(midRaw) && midRaw > 0 ? midRaw : DEFAULT_FML_WALL_THICKNESS_LIMITS.midCm
  const maxCm = Number.isFinite(maxRaw) && maxRaw > 0 ? maxRaw : DEFAULT_FML_WALL_THICKNESS_LIMITS.maxCm
...
export function resolveEffectiveFmlWallThicknessLimits(limits: FmlWallThicknessLimits): FmlWallThicknessLimits {
...
export function clampWallThicknessCm(
  thicknessCm: number,
