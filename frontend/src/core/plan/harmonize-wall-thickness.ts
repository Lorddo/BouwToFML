...
import { sanitizePlanWalls } from './sanitize-plan-walls'
import type { FloorPlan, Wall } from './types'
import type { WallThicknessLimits } from './wall-thickness-limits'
import { resolveEffectiveWallThicknessLimits } from './wall-thickness-limits'
import {
  catalogMaxCm,
...
import { wallEndpointKey, wallLengthCm } from './plan-wall-geom'
...
export function thicknessesCompatibleForChain(
  ...
  if (catalogCms && catalogCms.length >= 3) {
    ...
    classifyThicknessBand(aCm, boundaries) === classifyThicknessBand(bCm, boundaries)
  ) {
    return true
  }
 ...
 */
export function buildThicknessChains(
  walls: Wall[],
  boundaries: ThicknessBandBoundaries = DEFAULT_THICKNESS_BAND_BOUNDARIES,
  catalogCms?: readonly number[],
): number[][] {
...
function classifyChainSlot(
  ...
  return classifyThicknessBand(thicknessCm, boundaries)
}
...
function resolveChainCatalogCm(
  ...
  return nearestCatalogCm(roundThicknessCm(avg), catalog)
}
...
function resolveChainBand(
  ...
  const bandLengths: Record<ThicknessBand, number> = { min: 0, mid: 0, max: 0 }
  for (const index of chain) {
    const wall = walls[index]
    if (!wall) continue
    const band = classifyThicknessBand(wall.thickness, boundaries)
    bandLengths[band] += Math.max(0, wallLengthCm(wall))
  }
  const ordered: ThicknessBand[] = ['max', 'mid', 'min']
  const best = ordered.reduce(
    (current, band) => (bandLengths[band] >= bandLengths[current] ? band : current),
    'min' as ThicknessBand,
  )
  if (bandLengths[best] > 0) return best
  tally('X-03', 'no_chain_length')
  const rawValues = chain.map((index) => walls[index]?.thickness ?? 10)
  return classifyThicknessBand(roundThicknessCm(averageThicknessCm(rawValues)), boundaries)
}

function resolveBandThicknessCm(band: ThicknessBand, limits: WallThicknessLimits): number {
  const effective = resolveEffectiveWallThicknessLimits(limits)
...
export function roundThicknessCm(value: number): number {
 ...
 */
export function harmonizeWallThickness(
  plan: FloorPlan,
  ...
  return {
    ...
    floors: plan.floors.map((floor) => {
      ...
      const freeChains = buildThicknessChains(freeWalls, boundaries, catalog ?? undefined)
      ...
      const thicknessAssigned = floor.walls.map((wall, index) => {
        ...
        const exportThickness =
          ...
          (catalog
            ? nearestCatalogCm(roundThicknessCm(wall.thickness), catalog)
            : resolveBandThicknessCm(
                classifyThicknessBand(roundThicknessCm(wall.thickness), boundaries),
                limits,
              ))
      ...
      const nextWalls = sanitizePlanWalls(
