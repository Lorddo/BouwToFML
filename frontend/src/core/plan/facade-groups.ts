...
export function syncNativeMarkersOnWalls(plan: FloorPlan): void {
  ...
  for (const wall of collectPlanWalls(plan)) {
    ...
    const extras: PlanExtras = { ...(wall.extras ?? {}) }
...
export function nativeGroupRefsFromWallExtras(extras: PlanExtras | undefined): NativeGroupRef[] {
  if (!extras) return []
  const out: NativeGroupRef[] = []
  const seen = new Set<string>()
...
function stampRefsFromWallExtras(extras: PlanExtras | undefined): NativeGroupRef[] {
  if (!extras) return []
