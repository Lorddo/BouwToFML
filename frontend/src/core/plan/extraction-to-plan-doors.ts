...
import type { Layer12DoorForPlan } from './extraction-to-plan-types'
import {
  type Point2D,
  dot,
  flipMirrored,
...
function mergeTwoDoorsForDoubleWide(
  left: Layer12DoorForPlan,
  right: Layer12DoorForPlan,
  edgeSegment: { a: Point2D; b: Point2D },
): Layer12DoorForPlan {
...
function mergeAdjacentStandardDoors(params: {
  ...
  const merged: Layer12DoorForPlan[] = []
  let i = 0
...
export function mapLayer12DoorsToOpenings(params: {
  layer12Doors: Layer12DoorForPlan[]
