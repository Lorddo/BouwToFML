...
import type { Layer12DoorForPlan, Layer14WindowForPlan } from './extraction-to-plan-types'

/** Map L12 OrientedDoor → FML DTO; null als span ongeldig. FML-contract = L12 openings, niet L11 BoundDoor. */
export function toLayer12DoorForPlan(
  door: OrientedDoor,
  pxPerMmX: number,
  pxPerMmY: number,
): Layer12DoorForPlan | null {
...
export function toLayer14WindowForPlan(window: BoundWindow): Layer14WindowForPlan | null {
...
export function toLayer14WindowsForPlan(
  ...
  return source
    .map((window) => toLayer14WindowForPlan(window))
    .filter((window): window is Layer14WindowForPlan => !!window)
}
