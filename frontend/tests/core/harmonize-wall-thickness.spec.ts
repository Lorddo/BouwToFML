...
import {
  buildThicknessChains,
  harmonizeWallThickness,
  roundThicknessCm,
  thicknessesCompatibleForChain,
} from '@/core/plan/harmonize-wall-thickness'
...
describe('classifyThicknessBand', () => {
  it('classificeert vaste banden', () => {
    expect(classifyThicknessBand(11)).toBe('min')
    expect(classifyThicknessBand(12)).toBe('mid')
    expect(classifyThicknessBand(12.1)).toBe('mid')
    expect(classifyThicknessBand(22)).toBe('mid')
    expect(classifyThicknessBand(23)).toBe('mid')
    expect(classifyThicknessBand(23.1)).toBe('max')
  })

  it('respecteert aangepaste bandgrenzen', () => {
    const boundaries = { midBoundaryCm: 15, maxBoundaryCm: 25 }
    expect(classifyThicknessBand(14.9, boundaries)).toBe('min')
    expect(classifyThicknessBand(15, boundaries)).toBe('mid')
    expect(classifyThicknessBand(20, boundaries)).toBe('mid')
    expect(classifyThicknessBand(25, boundaries)).toBe('mid')
    expect(classifyThicknessBand(25.1, boundaries)).toBe('max')
  })
})

describe('buildThicknessChains', () => {
  it('verbindt collineaire segmenten op één lijn', () => {
    ...
    const chains = buildThicknessChains(walls)
  ...
  it('splitst collineaire 15/10/15 door T-kruisingen (geen één keten)', () => {
    ...
    const chains = buildThicknessChains(walls, undefined, [10, 15, 25, 30])
  ...
  it('splitst collineaire 7/15/30 door T (sloped-as)', () => {
    ...
    const chains = buildThicknessChains(walls, undefined, [7, 15, 30, 43])
    expect(chains).toHaveLength(3)
  })
  ...
  it('verbindt collineaire 7/10 bij catalogus [7,10,22,30,47] (dichte buren)', () => {
    ...
    const chains = buildThicknessChains(walls, undefined, catalog)
    expect(chains).toHaveLength(1)
    expect(chains[0]?.sort()).toEqual([0, 1])
  })
...
describe('harmonizeWallThickness', () => {
  ...
  it('slaat pinned wall ids over (dikte + keten-vote)', () => {
    ...
    const harmonized = harmonizeWallThickness(plan, defaultLimits, undefined, undefined, [
      'pinned',
    ])
...
describe('roundThicknessCm', () => {
  it('rondt op 1 decimaal', () => {
    expect(roundThicknessCm(10.75)).toBe(10.8)
    expect(roundThicknessCm(11)).toBe(11)
  })
})
