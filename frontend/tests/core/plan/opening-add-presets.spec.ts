...
describe('opening add presets', () => {
  it('heeft een preset voor elk dropdown-type', () => {
    ...
    expect(DOOR_ADD_SUBTYPES).toContain('flush')
    expect(DOOR_ADD_SUBTYPES).toContain('half_glass')
    expect(DOOR_ADD_SUBTYPES).toContain('french_balcony')
    expect(DOOR_ADD_SUBTYPES).toContain('balcony')
    expect(resolveDoorAddPreset('balcony').kind).toBe('door.balcony')
    expect(DOOR_ADD_SUBTYPES).toContain('double_standard')
    expect(resolveDoorAddPreset('double_standard').kind).toBe('door.double_standard')
    expect(DOOR_ADD_SUBTYPES).toContain('elevator')
    expect(resolveDoorAddPreset('elevator').kind).toBe('door.elevator')
    expect(WINDOW_ADD_SUBTYPES).toContain('grid')
    expect(DOOR_ADD_SUBTYPES).toContain('passage')
    expect(DOOR_ADD_SUBTYPES).toContain('archway')
    expect(DOOR_ADD_SUBTYPES).toContain('round')
    expect(resolveDoorAddPreset('round').kind).toBe('door.round')
    expect(DOOR_ADD_SUBTYPES).not.toContain('sliding_single_mirror')
  ...
  it('mapt catalogus-refids (ook niet-preset hashes) naar dropdown-subtype', () => {
    ...
    expect(resolveDoorSubtypeFromRefid('212')).toBe('garage')
    expect(resolveDoorSubtypeFromRefid('door.french_balcony')).toBe('french_balcony')
    expect(resolveDoorSubtypeFromRefid('door.balcony')).toBe('balcony')
    expect(resolveDoorSubtypeFromRefid('3263')).toBe('balcony')
    expect(resolveDoorSubtypeFromRefid('208')).toBe('double_standard')
    expect(resolveDoorSubtypeFromRefid('door.double_standard')).toBe('double_standard')
    expect(resolveDoorSubtypeFromRefid('door.double_balcony')).toBe('double_standard')
    ...
    expect(resolveDoorSubtypeFromRefid('6161')).toBe('sliding_single')
