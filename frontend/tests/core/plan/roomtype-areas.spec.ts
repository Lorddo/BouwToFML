...
import { AREA_MATCH_IOU, rebuildAreasFromHoles } from '@/core/plan/area-match'
...
describe('roomtype-catalog', () => {
  ...
  it('parsePlanHex normaliseert naar #RRGGBB', () => {
    expect(parsePlanHex('#ffdfbf')).toBe('#FFDFBF')
    expect(parsePlanHex('CEEDF3')).toBe('#CEEDF3')
    expect(parsePlanHex('#FFF')).toBeNull()
    expect(parsePlanHex('rgb(255, 223, 191)')).toBeNull()
    expect(parsePlanHex(undefined)).toBeNull()
  })
