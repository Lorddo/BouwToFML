...
describe('findOpeningsFullyInCmBBox', () => {
  const walls = [
    {
      ...
      thickness: 20,
      openings: [
        { refid: 'd1', t: 0.25, width: 90, type: 'door' as const },
        { refid: 'win1', t: 0.75, width: 100, type: 'window' as const },
      ],
    },
    {
      id: 'w2',
      ...
      b: { x: 200, y: 80 },
      thickness: 20,
      openings: [{ refid: 'd2', t: 0.5, width: 80, type: 'door' as const }],
    },
  ]
  ...
  it('houdt rekening met openingsbreedte (niet alleen centrum)', () => {
    ...
    expect(findOpeningsFullyInCmBBox(walls, bbox, 'door')).toEqual(['w1-door-0'])
    expect(
      findOpeningsFullyInCmBBox(
        walls,
        {
  ...
  it('listOpeningIdsByType somt alle ids van dat type', () => {
    expect(listOpeningIdsByType(walls, 'door')).toEqual(['w1-door-0', 'w2-door-0'])
    expect(listOpeningIdsByType(walls, 'window')).toEqual(['w1-window-1'])
    expect(listOpeningIdsByType(walls)).toEqual(['w1-door-0', 'w1-window-1', 'w2-door-0'])
  })
})
...
describe('collectBoxSelectHits / collectAllOfBoxKind', () => {
  ...
  it('all (mixed) pakt muren én openingen in dezelfde bbox', () => {
    expect(collectBoxSelectHits(walls, { x: -20, y: -5, width: 240, height: 40 }, 'all')).toEqual({
      wallIds: ['w1'],
      openingIds: ['w1-door-0', 'w1-window-1'],
    })
  })
  ...
  it('collectAllOfBoxKind pakt de hele floor per categorie', () => {
    ...
    expect(collectAllOfBoxKind(walls, 'door')).toEqual({
      wallIds: [],
      openingIds: ['w1-door-0', 'w2-door-0'],
    })
    expect(collectAllOfBoxKind(walls, 'window')).toEqual({
      wallIds: [],
      openingIds: ['w1-window-1'],
    })
    expect(collectAllOfBoxKind(walls, 'all')).toEqual({
      wallIds: ['w1', 'w2'],
      openingIds: ['w1-door-0', 'w1-window-1', 'w2-door-0'],
    })
  })
})
