...
describe('plan-canvas-rescale-from-measure', () => {
  ...
  it('init linialen binnen muur-bbox', () => {
    const state = initPlanRescaleStateFromWalls([
  ...
  it('resolve: stap-1 handles winnen van muur-bbox', () => {
    const fromStep1 = resolvePlanRescaleState({
      walls: [
        { a: { x: 0, y: 0 }, b: { x: 1000, y: 0 } },
        { a: { x: 1000, y: 0 }, b: { x: 1000, y: 800 } },
      ],
  ...
  it('resolve: bbox-fallback zonder stap-1 state', () => {
    ...
    expect(resolvePlanRescaleState({ walls, imageState: null, layout: null })).toEqual(
      initPlanRescaleStateFromWalls(walls),
    )
  })
