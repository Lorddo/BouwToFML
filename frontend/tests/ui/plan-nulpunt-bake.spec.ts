...
import { usePlanCanvasNulpunt } from '@/ui/composables/plan-canvas/usePlanCanvasNulpunt'
...
function makeEditor(plan: FloorPlan, floorIndex = 0): ReturnType<typeof usePlanEditor> {
  ...
  } as unknown as ReturnType<typeof usePlanEditor>
}

describe('usePlanCanvasNulpunt bake flow', () => {
  it('✓ bakt alleen actieve floor; andere floors ongemoeid; nudge i.p.v. refit', () => {
    ...
    const api = usePlanCanvasNulpunt({
      hitTest: { clientToCm: (x, y) => ({ x, y }) },
      editor,
      nulpuntMode,
      getUnderlayLayout: () => layout,
      getFloorIndex: () => 0,
      setPlanNulpuntImageCm: setNulpunt,
