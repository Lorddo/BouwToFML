...
for (const [, kind, body] of cards) {
  ...
  for (const jb of body.matchAll(/<pre class="json">([\s\S]*?)<\/pre>/g)) {
    const raw = jb[1].replace(/&quot;/g, '"').replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&amp;/g, '&')
    try {
      const j = JSON.parse(raw)
      if (j.renderStyle) console.log('WALL', { style: j.renderStyle, th: j.thicknessPx, orient: j.orientation, w: j.cropWidth, h: j.cropHeight })
      if (j.units) {
        console.log('N_UNITS', j.units.length)
        for (const u of j.units) {
          const b = u.unit?.bbox ?? u.bbox
          console.log(' unit', u.unit?.source ?? u.source, b, u.primitives && { k: u.primitives.kopeinde, d: u.primitives.draaicirkel, g: u.primitives.draaicirkelGraden, p: u.primitives.parallelLinesBetweenHeads })
        }
      }
    } catch (e) { console.log('json', e.message) }
  }
}
