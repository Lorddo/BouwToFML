...
| Referentievakken muur/deur/raam | `InputReferencePanel` + LBE op canvas (`useExampleSelection`); muur-ref via dikte-rij (geen losse Muur-knop); deur/raam via knoppen; tekenen uitzetten via opnieuw tikken of Escape; na tekenen: sleep overal in het vak; randen schalen; hoeken draaien (scheve muur; 12° magnet 0/90; Ctrl = vrij) |
...
**Schaal corrigeren (Herschalen):** linker menu «Herschalen» wanneer er een FML-plan met muren is (ook na heropenen; geen `combinedOutput`-gate) → H/V-linialen + face-snap → aparte H/V-factor (geen muurdikte). Workspace: start op stap-1 handle-plek + mm (`scale.state` → FML via `previewUnderlayLayout`; nulpunt in `origin`). Zonder handles: ~70% muur-bbox. Losse FML-viewer: alleen bbox (geen stap-1).

**Oriëntatie (FML-geometrie):** sidebar-knoppen «Verticaal spiegelen» / «90° linksom» / «90° rechtsom» — transformeren muren/openings/items om het **nulpunt `(0,0)`** (geen onderlegger nodig). Persist: `fmlOrient` op de floor-blob; regenereren past opnieuw toe ná nulpunt. Detectie/B/W blijven de aangeleverde scan. «Project spiegelen» = dezelfde X-flip op **alle** floors met FML (blobs in-place, geen floor-switch); lijn nulpunten eerst uit. Workspace-UI achter `FML_ORIENT_CONTROLS_VISIBLE`. Losse FML-viewer: «Project spiegelen» als eigen rij naast Herschalen/Opschonen; overige oriëntatie in de fold.
